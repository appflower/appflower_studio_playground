...
abstract class BaseResourceAttributeVersion extends BaseObject  implements Persistent
{
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				ResourceAttributeVersionQuery::create()
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(ResourceAttributeVersionPeer::ID) ) {
	 ...
	 * If this ResourceAttributeVersion is new, it will return
		...
		if(null === $this->collResourceAttributeVersionHashs || null !== $criteria) {
			if ($this->isNew() && null === $this->collResourceAttributeVersionHashs) {
				...
				$collResourceAttributeVersionHashs = ResourceAttributeVersionHashQuery::create(null, $criteria)
					->filterByResourceAttributeVersion($this)
					->find($con);
				if (null !== $criteria) {
	...
	{
		$query = ResourceAttributeVersionHashQuery::create(null, $criteria);
		$query->joinWith('ResourceVersion', $join_behavior);
	...
	{
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseResourceAttributeVersion:' . $name))
		{
